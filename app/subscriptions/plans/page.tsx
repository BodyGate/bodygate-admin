...
function uniqueBranchIds(plans: SubscriptionPlan[]) {
  return Array.from(
    new Set(
      plans
...
export default function SubscriptionPlansPage() {
  ...
  const branchOptions = useMemo<BranchOption[]>(() => {
    ...
    const fallbackBranches = uniqueBranchIds(plans)
      .filter((branchId) => !existingIds.has(branchId))
      .map((branchId, index) => ({
        id: branchId,
        name: `Sede piani esistenti ${index + 1}`,
        isFallback: true,
      }));

    return [...branches, ...fallbackBranches];
  }, [branches, plans]);
  ...
  async function initializePage() {
    ...
    if (branchError) {
      setBranches([]);
      setMessage("Sedi non caricate: visualizzo comunque i piani disponibili.");
    } else {
      setBranches(branchData || []);
    }

    await loadPlans("", { autoSelectSingleBranch: true });
  }

  async function loadPlans(
    branchId = "",
    options: { autoSelectSingleBranch?: boolean } = {}
  ) {
    setLoading(true);
    setErrorMessage("");
    ...
    if (error) {
      ...
      const planBranchIds = uniqueBranchIds(loadedPlans);

      setPlans(loadedPlans);
      setNewPlan((current) => ({
        ...current,
        sortOrder: String(loadedPlans.length + 1),
      }));

      if (
        options.autoSelectSingleBranch &&
        !selectedBranchId &&
        planBranchIds.length === 1
      ) {
        setSelectedBranchId(planBranchIds[0]);
      }
    }
        ...
        >
          ...
          {branchOptions.map((branch) => (
            <option key={branch.id} value={branch.id}>
              {branch.name || "Sede BodyGate"}{branch.city ? ` · ${branch.city}` : ""}{branch.isFallback ? " · da piani esistenti" : ""}
            </option>
          ))}
        </select>
      </section>
            ...
            <div className="plans-card-head">
              <div>
                ...
                <p>
                  ...
                  {selectedBranch
                    ? ` · ${selectedBranch.name || "Sede BodyGate"}`
                    : " · tutte le sedi"}.
                </p>
              </div>
