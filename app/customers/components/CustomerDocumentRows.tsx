  ...
  return <div className="doc-panel">
    ...
    {rows.map((row) => {
      ...
      return <div className="row" key={row.type}>
        ...
        <div className="actions"><button type="button" className="primary" onClick={() => setActive(row)} disabled={busyType === row.type}>Scatta</button><button type="button" onClick={() => setActive(row)} disabled={busyType === row.type}>Carica</button><button type="button" onClick={() => url && window.open(url, "_blank", "noopener,noreferrer")} disabled={!url}>Visualizza</button><button type="button" onClick={() => setActive(row)} disabled={busyType === row.type}>{url || doc ? "Sostituisci" : "Sostituisci"}</button></div>
      </div>;
    })}
