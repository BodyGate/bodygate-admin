...
export default function DocumentScannerDrawer({ open, title, documentType, initialFile, initialMode, operationMode = "create", initialValidFrom, initialValidUntil, onClose, onConfirm }: Props) {
  ...
  return (
    <div className="scanner-backdrop" role="dialog" aria-modal="true">
      <style jsx>{`
        .scanner-backdrop { position: fixed; inset: 0; z-index: 80; background: rgba(0,0,0,.74); display: flex; justify-content: flex-end; color: #fff; overflow: hidden; }
        .drawer { width: min(560px, 100%); height: 100dvh; max-height: 100dvh; min-height: 0; background: linear-gradient(145deg,#111,#050505); border-left: 1px solid #2b2b2b; display: grid; grid-template-rows: auto minmax(0,1fr) auto; box-shadow: -24px 0 60px rgba(0,0,0,.45); overflow: hidden; }
        .top { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; padding:18px 18px 14px; border-bottom:1px solid #242424; background:linear-gradient(145deg,#111,#070707); }
        ...
        .actions { position:sticky; bottom:0; z-index:2; display:grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap:10px; padding:12px 18px calc(12px + env(safe-area-inset-bottom)); border-top:1px solid #2b2b2b; background:linear-gradient(180deg,rgba(8,8,8,.96),#050505); box-shadow:0 -18px 36px rgba(0,0,0,.34); } .actions .wide { grid-column:1 / -1; }
