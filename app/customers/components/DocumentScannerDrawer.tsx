...
export default function DocumentScannerDrawer({ open, title, documentType, initialFile, initialMode, operationMode = "create", initialValidFrom, initialValidUntil, onClose, onConfirm }: Props) {
  ...
  return (
    <div className="scanner-backdrop" role="dialog" aria-modal="true">
      <style jsx>{`
        .scanner-backdrop { position: fixed; inset: 0; z-index: 80; width: 100%; height: 100vh; height: 100dvh; background: rgba(21,22,28,.45); display: flex; justify-content: flex-end; color: var(--text); overflow: hidden; }
        .drawer { width: min(560px, 100%); height: 100vh; max-height: 100vh; height: 100dvh; max-height: 100dvh; min-height: 0; background: var(--panel); border-left: 1px solid var(--border); display: grid; grid-template-rows: auto minmax(0,1fr) auto; box-shadow: -20px 0 60px rgba(21,22,28,.16); overflow: hidden; }
        .top { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; padding:18px 18px 14px; border-bottom:1px solid var(--border); background:var(--panel); } .top-actions { display:flex; align-items:center; gap:8px; flex:0 0 auto; } .top-actions button { min-height:44px; }
        .body { min-height:0; overflow-y:auto; overscroll-behavior:contain; -webkit-overflow-scrolling:touch; padding:14px 18px 16px; display:grid; gap:12px; align-content:start; scrollbar-width:thin; scrollbar-color: rgba(21,22,28,.2) transparent; }
        .body::-webkit-scrollbar { width:10px; } .body::-webkit-scrollbar-track{background:transparent}.body::-webkit-scrollbar-thumb{background:rgba(21,22,28,.2);border-radius:999px;border:2px solid var(--panel)}
        h2 { margin:0; font-size:22px; letter-spacing:-.03em; } p { margin:5px 0 0; color:var(--muted); font-size:13px; }
        button { border:1px solid var(--border); background:var(--bg-soft); color:var(--text); border-radius:14px; padding:12px 14px; font-weight:900; cursor:pointer; }
        button:disabled { opacity:.45; cursor:not-allowed; }
        .primary { background:var(--accent); border-color:var(--accent); color:#fff; } .ghost { background:var(--panel-2); }
        .file-action { position: relative; border: 1px solid var(--border); background: var(--bg-soft); color: var(--text); border-radius: 14px; padding: 12px 14px; font-weight: 900; cursor: pointer; text-align: center; min-height: 46px; display: inline-flex; align-items: center; justify-content: center; -webkit-tap-highlight-color: transparent; touch-action: manipulation; }
        .file-action.primary-action { background: var(--accent); border-color: var(--accent); color: #fff; }
        ...
        .stage { border:1px solid var(--border); border-radius:22px; min-height:330px; max-height:48vh; background: var(--panel-2); display:flex; align-items:center; justify-content:center; overflow:hidden; padding:14px; }
        ...
        .empty, .pdf { text-align:center; color:var(--muted); display:grid; gap:8px; } .pdf b { color:var(--text); font-size:28px; }
        .actions { position:relative; z-index:2; min-height:0; max-height:46vh; overflow-y:auto; overscroll-behavior:contain; -webkit-overflow-scrolling:touch; display:grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap:10px; padding:12px 18px calc(12px + env(safe-area-inset-bottom)); border-top:1px solid var(--border); background:var(--panel); box-shadow:0 -8px 24px rgba(21,22,28,.08); } .actions .wide { grid-column:1 / -1; }
        .validity{border:1px solid var(--border);border-radius:18px;padding:12px;display:grid;gap:10px;background:var(--panel-2)}.validity label{display:grid;gap:6px;color:var(--muted);font-size:12px;font-weight:900}.validity input{color-scheme:light;background:var(--bg-soft);border:1px solid var(--border);border-radius:12px;color:var(--text);padding:10px;font-weight:800}.quick{display:flex;gap:8px;flex-wrap:wrap}.summary{border-radius:14px;padding:10px;font-size:12px;line-height:1.45;background:rgba(37,99,235,.08);color:#1d4ed8}.summary.valid,.summary.expiring_soon{background:rgba(31,157,107,.1);color:var(--success)}.summary.expired{background:rgba(214,49,74,.1);color:var(--danger)}.summary.pending{background:rgba(37,99,235,.08);color:#1d4ed8}.summary.missing{background:rgba(179,121,10,.1);color:var(--warning)}.warning{color:var(--warning);font-weight:900;font-size:13px}.error { color:var(--danger); font-weight:800; font-size:13px; } .success { color:var(--success); font-weight:800; font-size:13px; }
