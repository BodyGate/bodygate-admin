...
// guidato <piano>"); showing it on every history row adds no information the
// row title doesn't already convey, so it's filtered out of the note display.
function isAutoGeneratedRenewalNote(note: string) {
  return /^rinnovo guidato\b/i.test(note.trim());
}
...
function SubscriptionHistoryRow({
  ...
  const note = String(subscription.notes || "").trim();
  const showNote = note && !isAutoGeneratedRenewalNote(note);
