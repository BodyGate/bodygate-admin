...
// Module-level so the standalone row components below (SubscriptionHistoryRow,
// MembershipFeeHistoryRow) can format dates without a raw ISO slice(0, 10).
function formatDateITModule(dateValue?: string | null) {
  if (!dateValue) return "-";
...
function paymentMethodLabelModule(method?: string | null) {
...
// The guided renewal flow always writes the same boilerplate note ("Rinnovo
// guidato <piano>"), which adds no information the row title doesn't already
// convey. A later reception edit/cancel appends its own note after it
// (joined with a blank line, see appendNote in the update-subscription API)
// rather than replacing it, so this strips only the boilerplate segment(s)
// and keeps any substantive note appended after it.
function stripAutoGeneratedRenewalNote(note: string) {
  return note
    .split(/\n{2,}/)
    .map((segment) => segment.trim())
    .filter((segment) => segment && !/^rinnovo guidato\b/i.test(segment))
    .join("\n\n");
}
...
function SubscriptionHistoryRow({
  ...
  const note = stripAutoGeneratedRenewalNote(String(subscription.notes || ""));
  ...
  return (
    <div className="history-row">
      <div className="history-main">
        ...
        <div className="history-period">
          {formatDateITModule(subscription.starts_at)} → {formatDateITModule(subscription.ends_at)}
          {subscription.payment_method
            ? ` · ${paymentMethodLabelModule(subscription.payment_method)}`
            : ""}
        </div>
        {showNote ? (
          <div className="history-note">{note.slice(0, 180)}</div>
        ) : null}
      </div>
  ...
  return (
    <div className="history-row">
      <div className="history-main">
        ...
        <div className="history-period">
          {formatDateITModule(fee.valid_from)} → {formatDateITModule(fee.valid_until)}
          {fee.payment_method
            ? ` · ${paymentMethodLabelModule(fee.payment_method)}`
            : ""}
        </div>
      </div>
      <div className="history-side">
