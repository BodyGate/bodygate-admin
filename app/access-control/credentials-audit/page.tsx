  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f4f5f8] text-zinc-900">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-6 px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        ...
        {loading ? <BGCard variant="premium"><div className="grid gap-4"><div className="bg-eyebrow">Audit in corso</div><div className="h-4 w-2/3 animate-pulse rounded-full bg-black/10" /><div className="h-24 animate-pulse rounded-3xl bg-indigo-500/10" /><p className="text-sm font-bold text-zinc-500">Analisi read-only di credenziali clienti, badge legacy e staff...</p></div></BGCard> : null}
        ...
        {summary && !loading && !error ? <>
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
            {kpis.map(([label, value, tone]) => <BGCard key={label} variant="soft" className="!p-5"><div className="text-xs font-black uppercase tracking-[0.16em] text-zinc-500">{label}</div><div className="mt-3 text-4xl font-black">{value}</div><div className="mt-3"><StatusBadge tone={tone} label={value ? "attenzione" : "ok"} /></div></BGCard>)}
          </section>
          <BGCard variant="premium"><div className="flex flex-wrap gap-2">{filters.map((item) => <button key={item.key} onClick={() => setFilter(item.key)} className={`rounded-2xl border px-4 py-3 text-sm font-black transition ${filter === item.key ? "border-indigo-500 bg-indigo-500/15 text-indigo-700" : "border-black/10 bg-black/[0.02] text-zinc-600 hover:border-indigo-400/50"}`}>{item.label}</button>)}</div>{payload.warnings?.length ? <p className="mt-4 text-sm font-bold text-amber-800">Warning tecnici: {payload.warnings.join(" · ")}</p> : null}</BGCard>
          ...
          {filter !== "ok" && !filtered.length ? <BGCard variant="success"><div className="text-center"><div className="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-3xl bg-emerald-400/15 text-3xl">✓</div><h2 className="text-3xl font-black">Nessuna anomalia rilevata</h2><p className="mt-2 text-sm font-bold text-zinc-500">La vista selezionata non contiene problemi da mostrare.</p></div></BGCard> : null}
          <section className="grid gap-4">{filtered.map((item) => <BGCard key={item.id} variant={item.risk === "critical" ? "danger" : item.risk === "medium" ? "warning" : "soft"}>
            <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-start"><div><div className="flex flex-wrap items-center gap-2"><StatusBadge tone={riskTone(item.risk)} label={item.risk === "critical" ? "Rischio alto" : item.risk === "medium" ? "Rischio medio" : "Rischio basso"} /><StatusBadge label={item.category} /><span className="rounded-2xl border border-black/10 bg-black/[0.04] px-3 py-2 font-mono text-sm font-black text-zinc-900">{item.code}</span></div><h2 className="mt-4 text-2xl font-black">{item.explanation}</h2><p className="mt-2 text-sm font-bold text-zinc-500">Azione consigliata: {item.recommended_action}</p></div><BGButton href={item.debug_url} variant="secondary">Apri nel Debug Center</BGButton></div>
            ...
            <div className="mt-4 rounded-2xl border border-black/10 bg-black/[0.03] p-4"><div className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Dettaglio tecnico sintetico</div><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{Object.entries(item.technical || {}).slice(0, 8).map(([key, value]) => <Info key={key} label={key.replaceAll("_", " ")} value={String(value ?? "—")} />)}</div></div>
          </BGCard>)}</section>
        </> : null}
      </div>
    </main>
...
function Info({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-black/10 bg-black/[0.03] p-4"><div className="bg-field-label">{label}</div><div className="mt-2 break-words text-sm font-extrabold text-zinc-900">{value || "—"}</div></div>; }
