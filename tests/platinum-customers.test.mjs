...
test("endpoint, header, payload e side effect Clienti restano invariati", async () => {
  ...
  assert.match(create, /result\.next_url \|\| `\/customers\/\$\{customerId\}\/contract`/)
  assert.match(create, /onboarding_warning/)
  assert.match(create, /router\.push\(`\$\{destination\}\$\{query\}`\)/)
